...
void rtcSetVBackup(int bCharge)
{
uint8_t ucTemp[4];

    if (iRTCType != RTC_RV3032) return; // only supported on RVxxxx devices

    ucTemp[0] = 0x11;
    ucTemp[1] = 0x4; // event interrupt enabled
    I2CWrite(RTC_RV3032_ADDR, ucTemp, 2);
    ucTemp[0] = 0x15;
    ...
    if (bCharge) { // enable trickle charging on VBAT pin
         ...
         I2CWrite(RTC_RV3032_ADDR, ucTemp, 3);
   ...
   if (iRTCType == RTC_RV3032) {
      if (iFreq == -1) { // disable it
          ...
          if (iFreq <= 32768) { // low speed
             ...
             i = udivmod32(iFreq, 8192000, NULL) - 1;
             if (i < 0) i = 0;
...
// Values are stored as BCD
        ...
        // seconds
        ucTemp[1] = rtcInt2BCD(pTime->tm_sec);
        // minutes
        ucTemp[2] = rtcInt2BCD(pTime->tm_min);
        // hours (and set 24-hour format)
        ucTemp[3] = rtcInt2BCD(pTime->tm_hour);
        // day of the week
        if (type == ALARM_DAY)
           ucTemp[4] = pTime->tm_wday + 1;
        // day of the month
        else if (type == ALARM_DATE) {
          ucTemp[4] = rtcInt2BCD(pTime->tm_mday);
        } else {
          ucTemp[4] = 0;
        }
...
// Values are stored as BCD
     ...
     switch (type)
     {
        ...
        case ALARM_DATE:
               ucTemp[0] = 0x08; // minutes alarm
               ucTemp[1] = rtcInt2BCD(pTime->tm_min); // first 7 bits hold BCD minutes
               ucTemp[2] = rtcInt2BCD(pTime->tm_hour);
               if (type == ALARM_TIME) {
                  ucTemp[3] = 0x80; // disable date alarm
               } else {
                  ucTemp[3] = rtcInt2BCD(pTime->tm_mday+1);
               }
               I2CWrite(iRTCAddr, ucTemp, 4);
               break;
         } // switch on alarm type
...
// Values are stored as BCD
        ...
        ucTemp[4] = pTime->tm_wday + 1;
        // day of the month
        ucTemp[5] = rtcInt2BCD(pTime->tm_mday);
        // month + century
        i = pTime->tm_mon+1; // 1-12 on the RTC
        ucTemp[6] = rtcInt2BCD(i);
	i = pTime->tm_year;
        if (i >= 100) {
...
uint8_t rtcBCD2Int(uint8_t u8BCD)
{
uint8_t u8Int;
	u8Int = (u8BCD >> 4) * 10;
	u8Int += (u8BCD & 0xf);
...
uint8_t rtcInt2BCD(uint8_t u8Int)
{
uint8_t u8BCD;
uint32_t u32Rem;
	u8BCD = udivmod32(u8Int, 10, &u32Rem);
	u8BCD <<= 4;
	u8BCD += u32Rem;
	return u8BCD;
} /* rtcInt2BCD() */
...
unsigned char ucTemp[20];
  ...
  if (iRTCType == RTC_DS3231) {
        ...
        my_memset(pTime, 0, sizeof(struct tm));
        // convert numbers from BCD
        pTime->tm_sec = rtcBCD2Int(ucTemp[0]);
        pTime->tm_min = rtcBCD2Int(ucTemp[1]);
        ...
        // month
        pTime->tm_mon = (((ucTemp[5] >> 4) & 1) * 10 + (ucTemp[5] & 0xf)) -1; // 0-11     
        pTime->tm_year = 100 + rtcBCD2Int(ucTemp[6]);
  }
} /* rtcGetTime() */
//
...
uint8_t ucClkDiv;
	...
	if (u32Millis < 1000) { // use fastest clock divider (1/4096 sec)
		...
		ucClkDiv = 1; // TD = 01
		iTicks = udivmod32((u32Millis * 64), 1000, NULL);
	} else if (u32Millis < 4095000) { // use 1 sec divider
		ucClkDiv = 2; // TD = 10
		iTicks = udivmod32(u32Millis, 1000, NULL); // 1 second per tick
	} else { // one minute per division
		ucClkDiv = 3; // TD = 11
		iTicks = udivmod32(u32Millis, 60000, NULL); // 1 minute per tick
	}
